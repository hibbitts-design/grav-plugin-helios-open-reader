...
The skeleton ships pre-configured as a single-publication reader with demo sections.
...
Rename section folders to match your content, either in the Admin Panel or via FTP. The number prefix on each folder (e.g. `01.section-1/`) controls the display order.
...
To host multiple publications within a single Helios Open Reader site, switch to multi-publication mode. A `publication-list.md` page serves as the publications home; each publication has its own `publication-home.md` and its section folders nested inside it. To set up multi-publication mode:
...
The publications home auto-detects all `publication-home` folders at root level and displays them as cards. To add another publication later:

1. Create a new numbered folder at root level (e.g. `03.another-publication/`)
2. Add a `publication-home.md` file inside it — set at minimum a `title`
3. Add section folders inside the publication folder
...
#### Publication Home Settings (`publication-home.md`)
