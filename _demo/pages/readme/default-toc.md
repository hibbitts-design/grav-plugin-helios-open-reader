...
- Plain text version link – a "Plain text version" footer link pointing to `/llms-full.txt`, Grav's native full-content Markdown export, for open access to all reader content in a portable, format-neutral form; useful for ebook generation (e.g. Pandoc), search and indexing tools, and AI-compatible tools. Each reader automatically gets its own `/<publication>/llms-full.txt` on multi-publication sites (requires Grav 2.1+); single-publication sites use Sitemap's site-wide file (shown when its "Serve llms-full.txt" setting is enabled)
