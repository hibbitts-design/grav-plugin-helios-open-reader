...
- **Plain text version link for open access** — an optional footer link to `/llms-full.txt`, Grav 2.1 and the Sitemap plugin's native full-content Markdown export, ready for search indexing, ebook pipelines, and any tool that can read a URL. On multi-publication sites, each reader can get its own scoped file instead of one file merging every reader together.
...
- Plain text version link — a configurable footer link with optional icon pointing to `/llms-full.txt`, Grav's native full-content Markdown export, for open access to all reader content in a portable, format-neutral form; useful for ebook generation (e.g. Pandoc), search and indexing tools, and AI-compatible tools. On multi-publication sites, each reader automatically gets its own `/<publication>/llms-full.txt` (requires Grav 2.1+); on single-publication sites, it's Sitemap's site-wide file (shown when the Sitemap plugin's "Serve llms-full.txt" setting is enabled)
...
| Show Plain Text Version Link in Footer | Disabled | Show a plain text version link in the page footer, pointing at `/llms-full.txt`. On multi-publication sites, each reader automatically gets its own file (requires Grav 2.1+ with Markdown Output enabled); on single-publication sites, uses the Sitemap plugin's site-wide file when its "Serve llms-full.txt" setting is enabled; not shown on the readers list page |
