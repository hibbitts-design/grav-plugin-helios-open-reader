...
- Plain text version link — a configurable footer link with optional icon pointing to `/llms-full.txt`, Grav's native full-content Markdown export, for open access to all reader content in a portable, format-neutral form; useful for ebook generation (e.g. Pandoc), search and indexing tools, and AI-compatible tools. On multi-publication sites, turning on Serve Per-Publication llms-full.txt gives each reader its own `/<publication>/llms-full.txt` (requires Grav 2.1+); on single-publication sites, it's Sitemap's site-wide file (shown when the Sitemap plugin's "Serve llms-full.txt" setting is enabled)
